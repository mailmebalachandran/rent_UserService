...
const saveUser = async (req, res) => {
  try {
    ...
    const checkUser = await userService.checkUserAlreadyExists(
      req.body.UserName
    );
    ...
    if (checkUser === undefined || checkUser === null) {
      ...
      return res
        .status(400)
        .send({ message: "The Username is already exists." });
    }
  } catch {
...
const authenticateUser = async (req, res) => {
  try {
    ...
    if (user != null || user != undefined) {
      let accessAndRefreshToken = await axios
        .post(process.env.AUTH_SERVICE_URL + "/authenticateUser", user)
        .then((res) => {
          return res.data;
        })
        .catch((err) => {
